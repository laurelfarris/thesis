...
;-
;- To Do:
;-   [] IMAGE_STRUCTURE has return statement followed by more code:
;-        find out what this code is supposed to do, then
;-        either fix it, put in different file/routine, delete completely.
;-      (10 April 2020)
;-
...
;        extra : { title : 'HMI continuum' + time + ' UT' } }
    ...
    READ_MY_FITS, index, data, $
        ; fls, ind=[],  $
        instr=instr, channel=channel, nodata=1, prepped=1
        ;year=year, month=month, day=day
        ;-  No need to pass these values now that script "@parameters"
        ;-     is called from most subroutines (10 April 2020)
    time = strmid(index.date_obs, 11, 5)
    ind = (where( time eq display_time ))[0]

    READ_MY_FITS, index, data, $
        instr=instr, channel=channel, nodata=0, prepped=1, ind=ind
    sz = size( data, /dimensions )
    X = (indgen(sz[0]) - index.crpix1) * index.cdelt1
    Y = (indgen(sz[1]) - index.crpix2) * index.cdelt2
    time = strmid(index.date_obs, 11, 11)
    ...
    READ_MY_FITS, index, data, instr=instr, $
        channel=channel, nodata=0, prepped=1, ind=ind
    ;print, 'Processing level = ', strtrim(index.lvl_num,1), ' for AIA ', channel
...
pro IMAGE_STRUCTURE, S, buffer=buffer
    ...
    wy = 5.5
    dw
    win = window( dimensions=[wx,wy]*dpi, buffer=buffer, location=[700,0] )
...
pro IMAGE_AR_ONLY, S, buffer=buffer
;- 12 February 2019
...
;save2, 'color_images';, /add_timestamp, idl_code='image_ar'
